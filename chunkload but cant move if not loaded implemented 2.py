    ...
    if workingOnChunks:
        ...
        for i in chunkNow:
            ...
            if chunks[i].activelength != chunks[i].length:
                chunksToNotWalkOn.update({i})
                workingOnChunks = True
        for j in chunkPrev:
            chunks[j].makeBlockInActive()
            if chunks[j].activelength != 0:
                chunksToNotWalkOn.update({j})
                workingOnChunks = True

    if (playerXchunked,playerZchunked) in chunksToNotWalkOn:
        ...
        modifiedPlayerPos = ( int((player.x / chunksize)), int((player.z / chunksize)) )

        if modifiedPlayerPos in chunksToNotWalkOn:
